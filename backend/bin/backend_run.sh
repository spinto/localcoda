...
if [[ $ORCHESTRATION_ENGINE == "local" ]]; then
  ...
  if [[ -n "$INSTANCE_USERNAME" ]]; then
    DOCKER_RUN_EXTRA="$DOCKER_RUN_EXTRA -l user=$INSTANCE_USERNAME"
  fi

  #Check if I have to pick up a port and if so, do so
  if [[ "$LOCAL_IPPORT" =~ RANDOM_PORT ]]; then
    #Pick a random port
    check_sw lsof
    RANDOM_PORT=
    while [[ -z "$RANDOM_PORT" ]]; do
      RANDOM_PORT=$(( ( RANDOM << 15 | RANDOM ) % 55536 + 10000 ))
      lsof -iTCP:$RANDOM_PORT -sTCP:LISTEN -t >/dev/null && RANDOM_PORT=
    done
    #Get the new ipport
    eval LOCAL_IPPORT=$LOCAL_IPPORT
    #Update the access accordingly
    EXT_MAINHOST="$EXT_MAINHOST:$RANDOM_PORT"
    EXT_PROXYHOST="$EXT_PROXYHOST:$RANDOM_PORT"
  fi
