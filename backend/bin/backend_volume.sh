...
Usage:
  backend_volume <command> [options]

Where <command> can be:
  init    Initialize the backend volume
...
if [[ "$COMMAND" == "init" ]]; then
  ...
  echo "Initializing tutorials volume..."
  ...
  if [[ -n "$TUTORIALS_VOLUME_ACCESS_MOUNT" ]]; then
    ...
    if [[ $ORCHESTRATION_ENGINE == "local" ]]; then
      ...
      echo "Initializing volume contents..."
...
EOF
  ...
  error 33 "invalid command specified. See help"
fi
